...
function layout(commits: CommitInfo[]): {
  rows: LaidOutCommit[];
  laneCount: number;
} {
...
function isImminent(item: QueueItem | undefined): boolean {
  return Boolean(
    item &&
    item.status === "armed" &&
    Date.parse(item.pushAt) - Date.now() <= 60_000,
  );
}
...
function renderHeader(): HTMLElement {
  ...
  if (state.repo) {
    ...
    branch.addEventListener("change", () =>
      post({ type: "LOAD_HISTORY", branch: branch.value }),
    );
  ...
  for (const [id, label, count] of defs) {
    const tab = el(
      ...
      [
        ...
        label,
        count !== null
          ? el("span", { class: "count num", text: String(count) })
          : null,
      ],
    );
...
function renderReelStrip(): HTMLElement {
  ...
  const allSelected =
    state.commits.length > 0 && state.selected.size === state.commits.length;
  strip.appendChild(
    button(
      ...
      () => {
        state.selected = allSelected
          ? new Set()
          : new Set(state.commits.map((c) => c.sha));
        syncSelection();
        render();
      },
...
function passesFilter(commit: CommitInfo): boolean {
  ...
  if (query) {
    const haystack =
      `${commit.subject} ${commit.sha} ${commit.authorName} ${commit.authorEmail}`.toLowerCase();
...
function renderReelScreen(): HTMLElement {
  ...
  if (state.loading.history && state.commits.length === 0) {
    list.appendChild(
      empty("...", "READING HISTORY", "Walking the git log for this branch."),
    );
    return screen;
  }
  if (state.commits.length === 0) {
    list.appendChild(
      empty("0", "EMPTY REEL", "This branch has no commits yet."),
    );
    return screen;
  }
  ...
  const maxFiles = Math.max(
    1,
    ...state.commits.map((c) => c.filesChanged ?? 0),
  );
  ...
  if (visible.length === 0) {
    list.appendChild(
      empty(
        "--",
        "NO MATCHES",
        "Nothing matches this search or filter. Clear a chip to widen it.",
      ),
    );
  } else {
      ...
      el("div", { class: "readout" }, [
        el("div", { class: "statline" }, [
          ...
          ...stats.map(([label, value, color]) =>
            el("span", { class: "stat num", style: `--c:${color}` }, [
              el("i"),
              `${label} ${value}`,
            ]),
          ),
...
function renderGutter(row: LaidOutCommit, width: number): HTMLElement {
  ...
  if (row.index > 0 && row.lanesBefore.includes(row.lane)) {
    svg.appendChild(
      svgEl("line", { class: "edge", x1: x, y1: 0, x2: x, y2: y }),
    );
  }
  if (row.parents.length > 0) {
    svg.appendChild(
      svgEl("line", { class: "edge", x1: x, y1: y, x2: x, y2: "100%" }),
    );
  }
...
function renderCommitRow(
  ...
  const node = el("div", {
    ...
    title:
      "Click to add to or remove from the queue · Shift-click selects a range",
  });
  ...
  node.appendChild(
    el("div", { class: "commit-main" }, [
      el("div", { class: "commit-line" }, [
        type
          ? el("span", { class: `type ${type}`, text: type.toUpperCase() })
          : null,
        el("span", {
      ...
      el("div", { class: "commit-meta" }, [
        ...
        el("span", { class: "sep" }),
        el("span", {
          title: new Date(row.authorDate).toLocaleString(),
          text: shortDate(row.authorDate),
        }),
  ...
  if (row.filesChanged !== undefined) {
    const fill = Math.max(
      8,
      Math.round(
        (Math.log(row.filesChanged + 1) / Math.log(maxFiles + 1)) * 100,
      ),
    );
    const pill = el(
      "span",
...
function renderCommitDetail(row: LaidOutCommit): HTMLElement {
  ...
  for (const file of files.slice(0, 500)) {
    list.appendChild(
      el("div", { class: "file-row" }, [
        ...
        el("span", {
          class: "add num",
          text: file.binary ? "bin" : `+${file.insertions}`,
        }),
        el("span", {
          class: "del num",
  ...
  if (files.length === 0) {
    list.appendChild(
      el("div", { class: "hint", text: "No file changes in this commit." }),
    );
  }
...
function renderMissionCard(): HTMLElement {
  ...
  const who = state.account
    ? `Signed in as ${state.account.login}`
  ...
  return el("section", { class: "card tight" }, [
    ...
    el(
      "div",
      { class: "target-line", title: `${target || "No target set"} · ${who}` },
      [
        el("span", { class: `conn-dot ${state.connection}` }),
        el("span", {
          class: "name",
          text: target ? prettyTarget(target) : "No target repository",
        }),
      ],
    ),
    el("div", { class: "chips-row" }, [
      ...
      el("span", {
        class: "mchip num",
        title: "Gap between pushes",
        text: `${state.staggerMinutes} MIN GAP`,
      }),
      el("span", {
        class: `mchip author-${state.author.mode}`,
        title: `Author: ${describeAuthor(state.author)}`,
        text:
          state.author.mode === "custom"
            ? "CUSTOM AUTHOR"
            : state.author.mode === "self"
              ? "AUTHORED BY ME"
              : "ORIGINAL AUTHOR",
      }),
    ]),
...
function renderProgressCard(): HTMLElement {
  ...
  return el("section", { class: "card tight" }, [
    el("h2", { class: "card-title" }, [
      "BROADCAST",
      el("span", { class: "aside", text: runStateLabel() }),
    ]),
    xp,
...
function renderUpNextCard(): HTMLElement {
  ...
  card.appendChild(
    el("h2", { class: "card-title" }, [
      "UP NEXT",
      items.length
        ? button("OPEN QUEUE", "sm blue", () => setTab("queue"))
        : null,
    ]),
  );
  ...
  if (items.length === 0) {
    card.appendChild(
      empty(
        "+",
        "NOTHING CUED",
        "Click commits on the reel to load them here.",
        true,
      ),
    );
    return card;
  ...
  if (items.length > ordered.length) {
    card.appendChild(
      el("div", {
        ...
        text: `+ ${items.length - ordered.length} more in the queue`,
      }),
    );
  }
  return card;
...
function renderOnAir(): HTMLElement {
  ...
  return el("div", { class: "onair-panel" }, [
    el("div", { class: "onair-row" }, [
      el("div", {
        class: `lamp ${deck === "live" ? "live" : deck === "armed" ? "armed" : ""}`,
      }),
      button(
        label,
        `lg orange${running && (pending.length || deck === "live") ? " running" : ""}`,
        confirmRun,
        {
...
function shortTime(item: QueueItem): string {
  switch (item.status) {
    ...
    case "armed": {
      ...
      return ms <= 60_000
        ? `FIRES ${clockTime(item.pushAt)}`
        : countdownTo(item.pushAt).toUpperCase();
    }
    case "cancelled":
...
function confirmRun(): void {
  ...
  state.modal = {
    ...
    summary: [
      [
        "TARGET",
        `${prettyTarget(run.targetRepoUrl)}@${run.targetBranch || "default branch"}`,
      ],
    ...
    button(
      ...
      () => {
        ...
        post({
          ...
          minutesBetween: state.staggerMinutes,
        });
      },
      {
        disabled: pendingItems().length === 0,
        title: "Refill push times in the current order",
      },
    ),
    el("div", { class: "grow" }),
    button("PAUSE", "yellow", () => post({ type: "PAUSE_RUN" }), {
      disabled: run?.state !== "running",
    }),
    button("CANCEL RUN", "orange", () => post({ type: "CANCEL_RUN" }), {
      disabled: !hasItems,
    }),
    button(
      "CLEAR QUEUE",
...
function renderQueue(): HTMLElement {
    ...
    scroll.appendChild(
      el("div", { class: "card", style: "max-width:560px;margin:40px auto" }, [
        empty(
          "[ ]",
          "QUEUE IS EMPTY",
          "Select commits on the REEL tab - each one you click lands here, ready to schedule.",
        ),
        el("div", { style: "display:flex;justify-content:center" }, [
          button("GO TO REEL", "blue", () => setTab("reel")),
        ]),
      ]),
    );
    return scroll;
  ...
  if (state.run?.pausedReason) {
    list.appendChild(
      el("div", { class: "qerror", text: state.run.pausedReason }),
    );
  }
  items.forEach((item, index) =>
    list.appendChild(renderQueueCard(item, index)),
  );
...
function renderQueueCard(item: QueueItem, index: number): HTMLElement {
  ...
  dateSelect.addEventListener("change", () => {
    ...
    post({
      ...
      patch: {
        ...
        customCommitDate:
          mode === "custom"
            ? (item.customCommitDate ?? item.originalDate)
            : undefined,
      },
    });
  });
  if (item.commitDateMode === "custom") {
    ...
    custom.addEventListener("change", () => {
      post({
        type: "UPDATE_QUEUE_ITEM",
        id: item.id,
        patch: { customCommitDate: fromLocalInput(custom.value) },
      });
    });
    dateField.appendChild(
      el("div", { class: "field-pair" }, [dateSelect, custom]),
    );
  } else {
  ...
  pushSelect.addEventListener("change", () => {
    post({
      type: "UPDATE_QUEUE_ITEM",
      id: item.id,
      patch: { pushMode: pushSelect.value as PushMode },
    });
  });
  ...
  if (item.pushMode === "relative") {
    const offset = el("input", {
      ...
      min: 0,
      step: 1,
      title: "Minutes after the previous push",
    }) as HTMLInputElement;
    ...
    offset.addEventListener("change", () => {
      post({
        ...
        id: item.id,
        patch: {
          relativeOffsetMinutes: Math.max(0, Number(offset.value) || 0),
        },
      });
    });
    pushField.appendChild(
      el("div", { class: "field-pair" }, [
        pushSelect,
        el("div", { class: "unit", "data-unit": "MIN" }, [offset]),
      ]),
    );
    ...
    when.addEventListener("change", () => {
      post({
        type: "UPDATE_QUEUE_ITEM",
        id: item.id,
        patch: { pushAt: fromLocalInput(when.value) },
      });
    });
    pushField.appendChild(
      el("div", { class: "field-pair" }, [pushSelect, when]),
    );
  } else {
  ...
  const clock = el("div", {
    class: `clock ${item.status}`,
    "data-clock-for": item.id,
  });
  ...
  if (item.error) {
    body.appendChild(
      el("div", { class: "qerror", text: truncate(item.error, 220) }),
    );
  }
  ...
  if (item.status === "failed") {
    actions.push(
      button("RETRY", "sm green", () =>
        post({ type: "RETRY_ITEM", id: item.id }),
      ),
    );
    actions.push(
      button("DETAILS", "sm", () => {
        state.modal = {
          title: `${item.shortSha} FAILED`,
          code: item.error ?? "No details recorded.",
        };
        render();
      }),
    );
    if (item.tempDir) {
      actions.push(
        button("OPEN TEMP FOLDER", "sm", () =>
          post({ type: "OPEN_TEMP_DIR", id: item.id }),
        ),
      );
    }
  }
  if (editable) {
    actions.push(
      button("PUSH NOW", "sm orange", () =>
        post({ type: "REPLAY_COMMIT", sha: item.sha }),
      ),
    );
    actions.push(
  ...
  switch (item.status) {
    ...
    case "armed": {
      ...
      time =
        ms <= 60_000
          ? `fires ${clockTime(item.pushAt)}`
          : `${countdownTo(item.pushAt)} · ${clockTime(item.pushAt)}`;
      break;
    }
    default:
...
function renderSetupStrip(): HTMLElement {
  return el("div", { class: "strip" }, [
    state.account
      ? el("span", {
          class: "label",
          text: `SIGNED IN AS ${state.account.login.toUpperCase()}`,
        })
      : button("SIGN IN TO GITHUB", "blue", () =>
          post({ type: "SIGN_IN_GITHUB" }),
        ),
    el("div", { class: "grow" }),
    button(
      state.showRepoPicker ? "HIDE REPOS" : "PICK FROM GITHUB",
      "purple",
      () => {
...
function renderTargetCard(): HTMLElement {
  ...
  const card = el("section", { class: "card" }, [
    el("h2", { class: "card-title" }, [
      ...
      el(
        "span",
        { class: "aside", style: "display:flex;align-items:center;gap:6px" },
        [el("span", { class: `conn-dot ${state.connection}` }), connLabel],
      ),
    ]),
    el("div", { class: "field" }, [
      el("label", { for: "target-url", text: "REPOSITORY" }),
      url,
    ]),
    el("div", { class: "field" }, [
      el("label", { for: "target-branch", text: "BRANCH · BLANK = DEFAULT" }),
      branch,
    ]),
  ]);
      ...
      if (state.repos.length === 0) {
        picker.appendChild(
          el("div", {
            class: "hint",
            style: "padding:12px",
            text: "No repositories you can push to.",
          }),
        );
      }
...
function renderSettingsCard(): HTMLElement {
  ...
  return el("section", { class: "card" }, [
    ...
    el("div", { class: "field" }, [
      ...
      el("div", {
        class: "hint",
        text: "0 pushes back to back. Applies to newly cued commits; use STAGGER on the QUEUE tab to refill existing rows.",
      }),
    ]),
  ]);
}
...
function renderAuthorCard(): HTMLElement {
  ...
  if (author.mode === "custom") {
    ...
    email.addEventListener("change", () =>
      commit({ email: email.value.trim() }),
    );

    card.appendChild(
      el("div", { class: "field" }, [
        el("label", { for: "author-name", text: "NAME" }),
        name,
      ]),
    );
    card.appendChild(
      el("div", { class: "field" }, [
        el("label", { for: "author-email", text: "EMAIL" }),
        email,
      ]),
    );
    ...
    if (problem) {
      card.appendChild(
        el("div", { class: "qerror", style: "margin-top:12px", text: problem }),
      );
    }
    card.appendChild(
      el("div", {
...
function renderRulesCard(): HTMLElement {
  const rules: [string, string, string, string][] = [
    [
      ...
      "Snapshots are extracted into a temp folder. Your branches, history and working tree are never written to.",
    ],
    [
      "ON",
      "var(--c-yellow)",
      "KEEP VS CODE OPEN",
      "Scheduled pushes fire from the extension. If VS Code was closed when one came due, you are asked before anything pushes.",
    ],
    [
      "!!",
      "var(--c-red)",
      "FAILURES HOLD THE QUEUE",
      "A failed push pauses the rest of the run. Retry it, then resume - nothing skips ahead silently.",
    ],
    [
      "Y",
      "var(--c-pink)",
  ...
  return el("section", { class: "card" }, [
    ...
    el(
      ...
      legend.map(([status, text]) =>
        el("div", {}, [
          el("span", { class: `pill ${status}`, text: status }),
          text,
        ]),
      ),
    ),
...
function renderLogStrip(): HTMLElement {
  ...
  return el("div", { class: "strip" }, [
    el("span", {
      class: "label num",
      text: `${c.sent} SENT · ${c.failed} FAILED`,
    }),
    el("div", { class: "grow" }),
...
function renderLog(): HTMLElement {
  ...
  const done = (state.run?.items ?? [])
    ...
    .sort(
      (a, b) =>
        Date.parse(b.completedAt ?? b.pushAt) -
        Date.parse(a.completedAt ?? a.pushAt),
    );
    ...
    scroll.appendChild(
      el("div", { class: "card", style: "max-width:560px;margin:40px auto" }, [
        empty(
          ">_",
          "NO BROADCASTS YET",
          "Every push that lands or fails is recorded here, with the SHA it became on the remote.",
        ),
      ]),
    );
    return scroll;
  ...
  table.appendChild(
    el("thead", {}, [
      el(
        "tr",
        {},
        ["STATE", "LOCAL → REMOTE", "COMMIT", "STAMPED", "PUSHED", ""].map(
          (h) => el("th", { text: h }),
        ),
      ),
    ]),
  ...
  for (const item of done) {
    ...
    body.appendChild(
      el("tr", {}, [
        ...
        el("td", {
          class: "subject",
          title: item.error ?? item.subject,
          text: item.subject,
        }),
        el("td", { class: "num", text: stamp }),
        el("td", {
          class: "num",
          text: item.completedAt ? clockTime(item.completedAt) : "-",
        }),
        el("td", {}, [action]),
      ]),
    );
...
function renderModal(): HTMLElement | null {
  ...
  const actions = el("div", { class: "modal-actions" }, [
    button(modal.confirm ? "CANCEL" : "CLOSE", "", close),
  ]);
  if (modal.confirm) {
...
function render(): void {
  ...
  const scrolls = new Map(
    SCROLLERS.map((id) => [id, document.getElementById(id)?.scrollTop ?? 0]),
  );
...
function showToast(
  ...
  const label =
    level === "error" ? "FAIL" : level === "warn" ? "HEADS UP" : "OK";
  const content = el("div", {}, [message]);
  if (level === "error") {
    const link = el("button", { class: "link", text: "SHOW DETAILS" });
    link.addEventListener("click", () => {
      const item = itemId
        ? state.run?.items.find((i) => i.id === itemId)
        : undefined;
      state.modal = {
        title: item ? `${item.shortSha} FAILED` : "ERROR",
        code: item?.error ?? message,
      };
      render();
    });
    content.appendChild(link);
  }
  const toast = el("div", { class: `toast ${level}` }, [
...
function tick(): void {
  ...
  for (const item of run.items) {
    ...
    if (isImminent(item)) {
      document
        .querySelector(`.commit-row[data-sha="${item.sha}"] .pulse-ring`)
  ...
  switch (message.type) {
    ...
    case "RUN_STATE": {
      ...
      if (
        nowState === "done" &&
        previousRunState &&
        previousRunState !== "done"
      ) {
        const c = message.run!.items.filter((i) => i.status === "sent").length;
        showToast(
          "info",
          `Broadcast complete - ${c} commit${c === 1 ? "" : "s"} sent.`,
        );
      }
    ...
    case "REPLAY_PROGRESS":
      if (message.status === "failed") {
        showToast(
          "error",
          `${message.sha.slice(0, 7)} failed to push. The rest of the queue is held.`,
          message.id,
        );
      }
      break;
