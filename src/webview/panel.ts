...
import { findRepoRoot, readCommitFiles, readHistory, readRepoInfo } from '../git/readHistory';
import type {
  AuthorConfig,
  CommitInfo,
  QueueOptions,
...
export class HackReplayPanel {
  ...
  static show(
    ...
    const panel = vscode.window.createWebviewPanel(VIEW_TYPE, 'Hack Replay', column, {
      enableScripts: true,

      retainContextWhenHidden: true,
      localResourceRoots: [vscode.Uri.joinPath(context.extensionUri, 'media')],
    });
    HackReplayPanel.instance = new HackReplayPanel(panel, context, scheduler, auth, log);
    return HackReplayPanel.instance;
  }
    ...
    this.disposables.push(
      this.scheduler.onDidChange((event) => {
        ...
        if (event.changedItemId && event.run) {
          const item = event.run.items.find((i) => i.id === event.changedItemId);
          if (item) {
            this.post({
    ...
    try {
      switch (message.type) {
        case 'READY':
          ...
          this.post({ type: 'GITHUB_SESSION', account: await this.auth.getAccount(false) });
          break;

        case 'LOAD_HISTORY':
        case 'REFRESH':
          await this.loadHistory('branch' in message ? message.branch : undefined);
          break;
        ...
        case 'LIST_GITHUB_REPOS': {
          ...
          try {
            this.post({ type: 'GITHUB_REPOS', repos: await this.auth.listRepos() });
          } finally {
        ...
        case 'CREATE_GITHUB_REPO': {
          const repo = await this.auth.createRepo(message.name, message.isPrivate);
          this.post({ type: 'GITHUB_REPOS', repos: [repo, ...(await this.auth.listRepos())] });
          this.post({ type: 'TOAST', level: 'info', message: `Created ${repo.fullName}.` });
          break;
        }
        ...
        case 'CHECK_TARGET': {
          ...
          this.post({ type: 'CONNECTION', state: repo ? 'signed-in' : 'unreachable' });
          if (!repo) {
            this.post({
              type: 'TOAST',
              level: 'warn',
        ...
        case 'STAGGER_QUEUE':
          await this.scheduler.stagger(message.ids, message.startAt, message.minutesBetween);
          break;
        ...
        case 'REPLAY_COMMIT': {
          const item = this.scheduler.run?.items.find((i) => i.sha === message.sha);
          if (item) {
        ...
        case 'OPEN_TEMP_DIR': {
          const item = this.scheduler.run?.items.find((i) => i.id === message.id);
        ...
        case 'SET_AUTHOR': {
          ...
          await config.update('customAuthorName', message.author.name.trim(), target);
          await config.update('customAuthorEmail', message.author.email.trim(), target);
          break;
        }
  ...
  private readConfig(): WebviewConfig {
    ...
    return {
      ...
      lastTargetRepoUrl: this.context.globalState.get<string>(LAST_TARGET_KEY, ''),
    };
  }
  ...
  private async loadHistory(branch?: string): Promise<void> {
    ...
    try {
      ...
      const maxCount = vscode.workspace.getConfiguration('hackReplay').get('maxCommits', 500);
  ...
  private async syncSelection(shas: string[], options: QueueOptions): Promise<void> {
    if (!this.repoPath) {
      return;
    }
    const selected = this.commits.filter((c) => shas.includes(c.sha));
    const targetRepoUrl = options.targetRepoUrl ? normalizeGitHubUrl(options.targetRepoUrl) : '';
    ...
    await this.scheduler.syncSelection(this.repoPath, this.sourceBranch, selected, {
      ...options,
      targetRepoUrl,
    });
  }
